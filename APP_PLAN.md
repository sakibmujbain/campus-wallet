...
- **Phase D — CR/organizer console: ✅ DONE & verified live.** Migration `0009` (event lifecycle `status`/`deadline`/`description`/`settled_txn_id`; `create_drive` — **scope-enforced** via `organizer_covers()` + roster auto-populate from the batch/club cohort; `add/remove_from_roster`, `set_drive_status`; `pay_event` **replaced** to reject payment to a non-`open` drive; `settle_event` — sweeps the pool to an **institutional wallet only**, idempotent by settled-status; `notification` spine + `remind_defaulters` + `mark_notifications_read`; all audited). `db/organizer.ts` + `db/notifications.ts`; API `/api/cr/drives`, `/api/cr/drives/[id]` (roster/status/refund/settle/remind), `/api/notifications/read`. Pages: `/cr` portfolio, `/cr/new` (scope-constrained), `/cr/drives/[id]` dashboard (roster+refund, defaulters+remind, lifecycle, settle), `/notifications` inbox + dashboard **bell**. Adversarial review (6-agent + verify): 13 raw → **6 confirmed** → all fixed — refund now threads a client **idempotency key** (no double-refund on retry); `remind_defaulters` deduped on unread-per-drive; `mapPgError` maps `40001/40P01`→409 and `22003/22007/22008`→422; read-committed mutations retry deadlocks; bounded numeric + real-date Zod validation. Migration `0010` closes refunds on a **settled** drive (its pool was swept to the treasury) with a clear 422 instead of a cryptic overdraft error, and the drive console hides refund/roster controls once settled. Smoke = **58 checks** green.
- **Phase E — Admin control-plane depth: ✅ DONE & verified live.** (Institution surface intentionally **deferred to post-demo**.) Migration `0011`: `admin_approve_kyc` + `admin_open_payee` — `is_admin`-gated wrappers around the pre-existing unguarded `approve_kyc`/`open_*_wallet`, audited; plus UNIQUE backstops (one payee per hall / exam department / till device). `db/adminops.ts`; API `/api/admin/kyc` + `/api/admin/wallets` (isAdmin-gated). Pages: **/admin/reconcile** (live `reconcile_balances()` FULL-OUTER-JOIN drift proof + treasury/pool/external), **/admin/audit** (append-only `audit_log` before→after diffs, per-table filter), **/admin/wallets** (payee directory + provisioning), **/admin/kyc** (approval queue + stored-vs-derived state machine). Adversarial review (4-agent + verify): 7 raw → **3 confirmed** → all fixed — **per-page `requireViewer("admin")`/`("cr")`** on every admin+organizer page (closes a stale-authz leak: a shared layout guard doesn't re-run on soft navigation, so a mid-session-demoted admin could keep reading); exact NUMERIC reconcile diff (no JS float); payee uniqueness (pre-checks + UNIQUE constraints). Smoke = **66 checks** green.
- **Phase F — Product polish (web app; offline shell deferred per stakeholder): ✅ DONE & verified.** Preceded by a 5-agent UI/UX audit (43 gaps; the app was already strongly polished — proactive top-up banners, consistent empty/busy states, exact money formatting). Added: shared **Skeleton** + **Progress** components and route-level `loading.tsx` on every force-dynamic route (hub/dues/events/admin/cr/drive); **error boundaries** (`error.tsx`/`global-error.tsx`/`admin/error.tsx`); friendly **money-path** failure (Payment Hub overdraft → "Not enough balance" + Top-up CTA); stripped dev-facing copy (`npm run db:seed`, Supabase); friendly **KYC status labels + guidance banners**; unified `money()` (dropped duplicate local copies); a11y (icon-button `aria-label`s, `aria-live` on all mutation results, bell unread in accessible name, `.sr-only`, chip `aria-pressed`/`aria-current`); responsive (`.scroll-x` on remaining tables, larger tap targets); and design-system DRY (`.msg.info` token, in-app two-step confirm replacing `confirm()` on Cancel/Settle, Settle empty-treasury guard, positive empty-queue states, `<a>`→`<Link>`). Type-check + build clean; smoke still **66** (no DB/API changes). **PWA install/offline intentionally skipped** (existing `manifest.ts`/`sw.js` left as-is).
  - **Follow-up — student /rewards + /savings surfaces (the two dead dashboard cards): ✅ DONE & verified.** These surface features that already existed in the DB. **/rewards**: points balance, **redeem points → BDT** (`redeem_points`, idempotency-key **required**), the `RANK()` leaderboard (matview read directly; refreshed after each redeem, not per page-load), point history. **/savings**: locked Tuition Shield balance + lock countdown, lifetime round-up count/total (a real `SUM`/`COUNT` aggregate, not the 20-row page), round-up history, and a **round-up tuner** (`set_savings_config`, migration `0012` — session-scoped writer, step 10/50, enable/disable; won't touch `locked_until`). Dashboard cards are now links. 4-agent review: 11 raw → **5 confirmed (all low)** → all fixed — leaderboard no longer `REFRESH`es per load, redeem idempotency key required, a `crypto.randomUUID`-in-insecure-context guard (`lib/idem.ts`, applied to every client form), a `NaNd ago` timestamp bug fixed by emitting ISO-Z from SQL (points **and** notifications), and the round-up tile aggregate. Smoke = **69** green.
  - Remaining: institution surface (post-demo).
