...
import { approveKyc } from "@/db/adminops";
import { mapPgError } from "@/lib/format";

export const runtime = "nodejs";

const Body = z.object({ verificationId: z.coerce.number().int().positive() });
...
export async function POST(req: Request) {
  ...
  try {
    await approveKyc(v.appUserId, parsed.data.verificationId);
